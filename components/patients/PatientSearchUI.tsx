...
export default function PatientSearchUI({ onSelect, onAddNew }: { onSelect: (p: Patient) => void, onAddNew: () => void }) {
        ...
        <ul className="absolute z-20 w-full mt-2 bg-white border border-clinic-border rounded-lg shadow-xl max-h-80 overflow-y-auto animate-in fade-in slide-in-from-top-2">
          ...
          {results.map(p => (
            <li key={p.id} onClick={() => { onSelect(p); setQuery(''); setResults([]); }} className="p-4 hover:bg-clinic-emerald/5 cursor-pointer border-b border-gray-50 flex items-center group transition-colors">
              ...
              <div>
                <p className="font-bold text-clinic-navy">{p.name}</p>
                <p className="text-sm text-gray-500 mt-0.5">{p.phone} • {p.age}y • {p.gender}</p>
              </div>
            </li>
          ))}
          <li onClick={onAddNew} className="p-4 bg-gray-50 hover:bg-clinic-emerald/10 cursor-pointer text-clinic-emerald flex items-center font-bold border-t border-gray-100 transition-colors">
